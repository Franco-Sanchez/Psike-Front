...
import { Redirect, useParams } from "react-router-dom";
 ...
 export default function ShowAppointment() {
  ...
  function FindAppointment(){
   ...
   return (
     <StyledAppoitment>
       <AvatarHeader name={user.name} lastname={user.lastname} onClick={()=>Kill()}/>
       {  FindAppointment().map((q)=>(
         <Heading3>Datos sobre tu cita con <span>{q.psychologist.name}</span>:</Heading3>
       ))
       } 
        ...
        {FindAppointment().map((q)=>(   
         ...
         <Code><ContentS>{q.transfer.code}</ContentS></Code>
       </AppoIcons>
         ))}


       <Heading5><p>Comentarios que te dio el psicologo:</p></Heading5>
       ...
       <PsicologyComent>
        ...
        <ContentM>
          {q.feedback}
           Lorem ipsum dolor sit amet consectetur adipiscing, elit sociis magnis
           donec velit iaculis, inceptos montes purus nascetur elementum.        Habitasse commodo himenaeos imperdiet nisi dignissim accumsan,           faucibus ridiculus ad vehicula erat, scelerisque convallis penatibus
           lobortis fusce. Ac cursus vel litora arcu porta magna pretium mi ante
           eu laoreet tellus, nibh condimentum cubilia pellentesque nulla         pulvinar sapien diam tristique varius consequat. Augue luctu           phasellus facilisi nullam fames a sem, nunc nam in dis leo lectus.
           Parturient aliquam congue platea ornare quam, taciti conubia metus
           hendrerit fringilla, curabitur interdum sed facilisis. Class eleifend
           molestie orci enim porttitor posuere tempor vitae nostra, fermentum         quisque natoque id volutpat cum primis turpis, dui venenatis egestas
           aliquet ultrices ullamcorper nisl dictumst. Integer mus mattis tortor,
           mollis aptent.
         </ContentM>
          ))}
       </PsicologyComent>
